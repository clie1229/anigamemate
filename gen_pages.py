# -*- coding: utf-8 -*-
"""
AnigameMate 기종 카테고리 페이지 일괄 생성기
- platforms.py 데이터를 읽어 retro-games-<slug>.html 을 생성
- 기준 템플릿: retro-games-famicom.html (동일 디자인/구조/구조화데이터)
- 패미컴은 손질된 4상품 버전이 이미 있으므로 기본 제외(--all 로 포함 가능)
"""
import sys, io, os, re
...
GRADE_SECTION = """  <section id="grade" style="background:#f4efe5">
    <div class="wrap">
      ...
      <table>
        ...
        <tbody>
          <tr><td><span class="grade mono">S</span></td><td>미개봉 또는 사용감 거의 없음</td><td>박스·설명서 완비</td><td>확인 완료</td></tr>
          <tr><td><span class="grade mono">A</span></td><td>경미한 사용감, 라벨 온전</td><td>박스 또는 설명서 일부</td><td>확인 완료</td></tr>
          <tr><td><span class="grade mono">B</span></td><td>흠집·변색 있음, 플레이 지장 없음</td><td>본품만</td><td>확인 완료</td></tr>
          <tr><td><span class="grade mono">부품용</span></td><td>상태 무관</td><td>본품만</td><td>미동작 (제목에 명시)</td></tr>
        </tbody>
      </table>
    </div>
  </section>"""
...
RELATED_SUB = {
    ...
    "세가": "세가 계열과 국내 유통판",
    "소니": "플레이스테이션 계열 기종",
    "국내 레트로": "국내 정식 유통 기종과 원본 기종",
    "기타 기종": "마니아 수요가 있는 기종",
}


def josa_heading(name):
    """about 섹션 제목: 한글로 끝나면 이란/란, 아니면 '<name> 소개'."""
    last = name[-1]
    code = ord(last)
    if 0xAC00 <= code <= 0xD7A3:
        has_batchim = (code - 0xAC00) % 28 != 0
        return name + ("이란" if has_batchim else "란")
    return name + " 소개"


def style_about_links(html):
    return html.replace('<a href=', '<a style="color:var(--brand);font-weight:600" href=')
...
def related_for(p):
    group = p["group"]
    same = [q for q in PLATFORMS if q["group"] == group and q["slug"] != p["slug"]]
    korea = [q for q in PLATFORMS if q["group"] == "국내 레트로" and q["slug"] != p["slug"]]
    picks = list(same[:3])
    for q in korea:
        if q not in picks and len(picks) < 5:
            picks.append(q)
    if len(picks) < 5:
        for q in PLATFORMS:
            if q["slug"] != p["slug"] and q not in picks:
                picks.append(q)
                if len(picks) >= 5:
                    break
    return picks[:5]
...
{{
  ...
  "itemListElement": [
    ...
    # ItemList
    items = []
    for i, (pn, sku, price, grade, chips, meta, stock) in enumerate(p["products"], start=1):
...
{NL.join(items)}
    ...
    faq_items = []
    for q, a in p["faqs"]:
        faq_items.append(f"""    {{ "@type": "Question", "name": "{q}",
      "acceptedAnswer": {{ "@type": "Answer", "text": "{a}" }} }}""")
    faqpage = f"""<script type="application/ld+json">
{{
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
...
def product_cards(p):
    cards = []
    for (pn, sku, price, grade, chips, meta, stock) in p["products"]:
        if stock:
            grade_label = f"{grade}등급" if grade in SAB else grade
            first = f'<span class="chip g">{grade_label}</span>'
        else:
            first = '<span class="chip off">품절</span>'
        chip_html = first + "".join(f'<span class="chip">{c}</span>' for c in chips)
        price_cls = "price soldout" if not stock else "price"
        if stock:
        ...
        cards.append(f"""          <a class="prod" href="/p/{sku}">
...
def about_section(p):
    heading = josa_heading(p["name"])
    paras = "\n        ".join(f"<p>{style_about_links(t)}</p>" for t in p["about"])
...
def faq_section(p):
    ...
    for idx, (q, a) in enumerate(p["faqs"]):
        openattr = " open" if idx == 0 else ""
        rows.append(f"""        <details{openattr}>
          <summary>{q}</summary>
          <p>{a}</p>
        </details>""")
    return f"""  <section id="faq" style="background:#f4efe5">
    <div class="wrap">
...
def related_section(p):
    ...
    sub = RELATED_SUB.get(p["group"], "함께 보는 기종")
...
def render(p):
    slug, name = p["slug"], p["name"]
    base = "https://anigamemate.com"
    title = f"{name} 중고 | 소프트·본체·주변기기 | AnigameMate"
    desc = (f"{name} 중고 소프트·본체·주변기기. 모든 상품을 실기 구동 확인 후 "
            f"S·A·B 등급으로 등록합니다. 박스풀셋·지역코드 표기, 부품용 별도 분류.")
    og_desc = f"{name} 중고. 실기 구동 확인 완료 + S·A·B 상태 등급 표기."
...
<link rel="canonical" href="{base}/retro-games/{slug}">

<meta property="og:type" content="website">
<meta property="og:site_name" content="AnigameMate">
<meta property="og:title" content="{title}">
<meta property="og:description" content="{og_desc}">
<meta property="og:url" content="{base}/retro-games/{slug}">
...
"""
    ...
    head_block = f"""    <div class="head">
      ...
      <nav class="tabs" aria-label="상품 유형">
        <a class="tab" href="retro-games-{slug}.html" aria-current="true">전체</a>
        <a class="tab" href="retro-games-{slug}-software.html">소프트</a>
        <a class="tab" href="retro-games-{slug}-console.html">본체</a>
        <a class="tab" href="retro-games-{slug}-accessories.html">주변기기</a>
        <a class="tab" href="retro-games-{slug}-parts.html">부품·수리용</a>
      </nav>
    </div>"""

    filters = f"""      <aside class="filters">
        <h3>필터</h3>
        <p class="hint">필터 결과 페이지는 색인되지 않고 이 카테고리로 canonical 처리됩니다.</p>
        <form action="/retro-games/{slug}" method="get">
          <fieldset>
            <legend>상태 등급</legend>
          ...
          <fieldset>
            <legend>동작 확인</legend>
            <label class="opt"><input type="checkbox" name="work" value="ok"> 확인 완료</label>
            <label class="opt"><input type="checkbox" name="work" value="no"> 부품용(미동작)</label>
          </fieldset>
          <fieldset>
            <legend>장르</legend>
            <label class="opt"><input type="checkbox" name="genre" value="action"> 액션</label>
            <label class="opt"><input type="checkbox" name="genre" value="rpg"> RPG</label>
            <label class="opt"><input type="checkbox" name="genre" value="shooting"> 슈팅</label>
            <label class="opt"><input type="checkbox" name="genre" value="sports"> 스포츠</label>
            <label class="opt"><input type="checkbox" name="genre" value="puzzle"> 퍼즐</label>
          </fieldset>
          <fieldset>
            <legend>가격대</legend>
    ...
    list_section = f"""  <section id="list">
    <div class="wrap layout">

{filters}

      <div>
        <h2 class="title">{name} 중고 상품</h2>
        <p class="sub">전체 {len(p['products'])}개 · 실기 동작 확인 완료</p>
        <div class="prods">
...
{product_cards(p)}
    ...
    alert = f"""  <section id="alert">
    <div class="wrap">
      <div class="cta-band">
        <h2>찾는 {name} 상품이 없나요?</h2>
...
<main>
  <div class="wrap">
    <nav class="crumb" aria-label="브레드크럼">
      <a href="index.html">홈</a> › <a href="retro-games.html">레트로게임 중고</a> › <span aria-current="page">{name}</span>
    </nav>
...
def main():
    ...
    made = []
    for p in PLATFORMS:
        if p["slug"] == "famicom" and not include_all:
            continue
        path = os.path.join(HERE, f"retro-games-{p['slug']}.html")
        with open(path, "w", encoding="utf-8") as f:
            f.write(render(p))
        made.append(p["slug"])
    print(f"생성 완료: {len(made)}개")
    for s in made:
        print("  retro-games-" + s + ".html")
